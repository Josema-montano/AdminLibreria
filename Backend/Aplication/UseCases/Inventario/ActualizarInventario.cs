...
namespace Aplication.UseCases.Inventario
{
 ...
 var prod = await _productoRepo.ObtenerPorIdAsync(productoId);
 if (prod == null) throw new ArgumentException($"Producto con ID {productoId} no encontrado");
 if (tipo==TipoMovimientoInventario.Egreso && prod.Stock < cantidad)
 throw new InvalidOperationException($"Stock insuficiente. Stock actual: {prod.Stock}, solicitado: {cantidad}");
