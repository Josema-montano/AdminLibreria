...
private readonly ListarTodosMovimientos _listarTodosMovimientos;
 ...
 ListarTodosMovimientos listarTodosMovimientos)
 {
    ...
    _listarMovimientosPorProducto = listarMovimientosPorProducto;
     ...
     _listarTodosMovimientos = listarTodosMovimientos;
  }
...
{
 var movimientos = await _listarTodosMovimientos.EjecutarAsync();
 if (!movimientos.Any())
 return Ok(new { mensaje = "No hay movimientos registrados." });
 return Ok(movimientos);
}
  }
}
