...
public async Task<IActionResult> AsignarProductos(int id, [FromBody] List<PromocionProductoDTO> productos)
{
 if (productos == null || !productos.Any())
 return BadRequest(new { mensaje = "Debe enviar al menos un producto." });
 var productosAsignar = productos.Select(p => (p.ProductoId, p.DescuentoPorcentaje));
 await _actualizarPromocion.AgregarProductosAsync(id, productosAsignar);
 return Ok(new { mensaje = "Productos asignados correctamente." });
}
    }
}
